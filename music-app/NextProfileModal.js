...
function NextProfileModal({ className, onClose, visible }) {
  ...
  const [selectedImage, setSelectedImage] = useState((process.env.PUBLIC_URL + imgRoute) !== "" ? imgRoute: "/img/EmptyUser.jpg");
  ...
  const withDrawal = ()=>{
    const result = window.confirm("회원 탈퇴하시겠습니까?");
    if(result){
        ...
        <ModalInner tapIndex="0" className="modal-inner" style={{height:modalHeight}}>
          ...
          <ImageContainer>
            <div style={{display: 'flex', alignItems: 'center' }}>
              <img className='mypage-user-img'style={{display: 'flex',marginLeft:"20px",marginTop:"20px"}} src={(process.env.PUBLIC_URL + imgRoute) !== "" ? imgRoute : "/img/EmptyUser.jpg"}/>
            ...
            <div style={{ marginLeft: '30px', verticalAlign: 'top' }}>
              ...
              <span style={{ fontWeight: 'normal', fontFamily: 'SC Dream 4', fontSize: '24px', color: '#ececec', display: 'block' }}>email:{email}</span>
            </div>
          </ImageContainer>
          <ButtonContainer>
            {isButtonVisible && (
          ...
          {modalpwIsOpen && (
            ...
            <div style={{ display:'flex'}}>
              ...
              <PwCheck onClick={changePassword}>확인</PwCheck>
            </div>
          </div>
          )}
